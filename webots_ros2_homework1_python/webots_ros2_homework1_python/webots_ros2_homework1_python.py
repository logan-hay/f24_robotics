...
COLLISION_WIDTH = 0.23

# Collision Check Constants
LIDAR_AVOID_DISTANCE = 0.77
...
WALL_CHECK_DISTANCE = WALL_FOLLOW_DISTANCE + DISTANCE_TOLERANCE + 0.25

# Space Check Constants
SPACE_CHECK_DEPTH = WALL_FOLLOW_DISTANCE + DISTANCE_TOLERANCE + 0.15
SPACE_DEAD_ZONE = 0.10
SAFE_TURN_DISTANCE = 0.30
OPENING_CHECK_OFFSET = -27

# Record Keeping Constants
SPAWN_POINT = 4
TRIAL = 5
RECORD_INTERVAL = 3
...
class RandomWalk(Node):
    ...
    def __init__(self):
        ...
        # Used for logging
        self.current_path = []
        self.output_file = f'trials/spawn_{SPAWN_POINT}/trial_{SPAWN_POINT}_{TRIAL}.trial'
    ...
    def listener_callback3(self, msg3):
        ...
        if current_time - self.last_recorded_time >= RECORD_INTERVAL:
    ...
    def timer_callback(self):
        ...
        if self.reverse_check(): # Reverse     
            ...
            if self.opening_check(): # Enter Opening (Turn Right)
                ...
                if(from_wall < WALL_FOLLOW_DISTANCE):
                    ...
                    if abs(front_distance - back_distance) > ANGLE_TOLERENCE:
                        if front_distance > back_distance:
                            self.cmd.angular.z = -0.125            
                            self.cmd.linear.x = 0.22
                            self.publisher_.publish(self.cmd)
                            self.get_logger().info('Following Wall (Foward - Soft Right)') 
                        else:
                            self.cmd.angular.z = 0.125              
                            self.cmd.linear.x = 0.22
                            self.publisher_.publish(self.cmd)
                            self.get_logger().info('Following Wall (Foward - Soft Left)') 
                    else:
                        self.cmd.linear.x = 0.3                   
                        self.cmd.linear.z = 0.0
                        self.publisher_.publish(self.cmd)
                        self.get_logger().info('Following Wall (Foward)')
                self.turtlebot_moving = True
                self.turtlebot_turning = False
        else:
            self.cmd.linear.x = 0.3
...
def main(args=None):
    ...
    try:
        ...
        random_walk_node.end_trial()  # Ensure last path is saved
